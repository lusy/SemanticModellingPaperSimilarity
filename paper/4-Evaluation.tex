...
Diese Arbeit berechnet die semantische Ähnlichkeit mit drei verschiedenen Gewichtungen, die in Absprache mit Experten aus dem Konrad-Zuse-Zentrum zustande gekommen sind.
...
Bei Gewichtung 1 wurden die Keywords am höchsten gewichtet und damit als am relevantesten zur Charakterisierung einer Publikation gewertet.
An Stelle zwei folgen die Zitationen, wodurch Publikationen durch direkte Verknüpfungen zu anderen Publikationen ebenfalls in ein Themenumfeld eingeordnet werden können.
Autoren, Quellen und Erscheinungsjahre werden als gleich relevant für die Publikation gewertet.
In Gewichtung 2 sind alle verfügbaren Metadaten einer Publikation gleich wichtig, unabhängig ob sie einen inhaltlichen oder einen eher beschreibenden Anteil besitzen.
In Gewichtung 3 schließlich werden Keywords und Autoren als am relevantesten gewertet, was eine stark inhaltliche Ausrichtung des Algorithmus bedeutet.
...
Das System, auf dem der von dieser Arbeit vorgeschlagene Algorithmus für die Berechnung der semantischen Ähnlichkeit zwischen Publikationen ausgeführt wird, verfügt über $2.93$GHz CPU und $14$Gb Arbeitsspeicher.
...
Es wurde die semantische Ähnlichkeit der \textit{zmath}-Publikationen bis 1975, für die alle Metadaten vorliegen, mit dem Dämpfungsfaktor $c=0.6$, $k=7$ Anzahl an Iterationen und drei verschiedenen Gewichtungsverteilungen (siehe Kapitel \ref{subsec:params}) berechnet.
Zur Evaluation des entwickelten Maßes wurde das \textit{C-Rank}-Verfahren auf demselben Datensatz (mit denselben Parametern $c=0.6$ und $k=7$) ausgeführt, um die Ergebnisse zu vergleichen.
...
\\
\\
In der Evaluation gilt es herauszufinden, inwiefern ähnliche Publikationen (d.h. ein vergleichsweise hoher Ähnlichkeitswert im Intervall \begin{math}\left[0,1\right]\end{math} zwischen je zwei Publikationen) auf die entsprechenden MSC-Klassen, die im Algorithmus nicht betrachtet wurden, abgebildet werden können.
Die MSC-Klassen ordnen die Publikationen thematisch ein und es ist zu erwarten, dass thematisch ähnliche Publikationen auch innerhalb der Klassifikation in ähnliche Klassen eingeordnet werden können.
\\
\\
Für die Clusterings der drei Parametrisierungen des semantischen Ähnlichkeitsmaßes, sowie des \textit{C-Rank} wurden die Werte \textit{Entropy}, \textit{Purity} und der \textit{Silhouette-Koeffizient} ermittelt.
\textit{Entropy} und \textit{Purity} sind Metriken der Supervised Cluster Validation \cite{dataMining2005}.
Wenn eine externe Klassifizierung der Objekte, auf die ein Cluteringverfahren angewandt werden sollte, vorliegt (in dem Fall die MSC-Klassifizierung), bestimmen \textit{Entropy} und \textit{Purity} inwiefern die dabei entstandenen Cluster nur aus Elementen bestehen, die derselben externen Klasse angehören.
...
Unsupervised Cluster Validation beurteilt inwiefern das Clustering an sich gut ist (d.h. ohne das Clustering mit einer externen Klassifizierung zu vergleichen), indem die Zusammengehörigkeit der Elemente innerhalb jedes Clusters gemessen wird und auch wie gut die einzelnen Cluster von einander abgegrenzt sind.
...
Der \textit{Silhouette-Koeffizient} $\in (-1,1)$ bestimmt inwiefern ein Element ähnlich allen anderen Elementen aus dem selben Cluster und gleichzeitig verschieden zu den Elementen aus anderen Clustern ist.
Ein negativer Wert dieses Koeffizienten bedeutet, dass das Element im Durchschnitt näher an Elementen ist, die außerhalb des Cluster liegen, als an Elementen, die sich im selben Cluster befinden.
\\
\\
...
Im Vergleich schneiden die drei semantischen Ähnlichkeitsmaße besser als \textit{C-Rank} ab.
\textit{C-Rank} hat den größten \textit{Entropy}-Wert und die kleinsten Werte für \textit{Purity} und \textit{Silhouette}.
Ferner lässt sich feststellen, dass unter den drei verschiedenen Metadatengewichtungen die Gewichtung 2, bei der alle Metadaten gleich gewichtet waren, am schlechtesten abgeschnitten hat.
Gewichtung 1 und Gewichtung 3 liefern ähnliche Resultate.
Gewichtung 3 hat im Vergleich zu den anderen 3 Methoden die besten Werte für \textit{Entropy} und \textit{Purity}, d.h. das Clustering bildet die Ähnlichkeitswerte am besten auf die MSC-Klassen ab.
...
Abbildung\footnote{Eine größere Version der Abbildungen befindet sich im Appendix} \ref{fig:entPur} stellt die Verteilungen der \textit{Entropy}- und \textit{Purity}-Werte für alle Metadatengewichtungen des semantischen Ähnlichkeitsmaßes und \textit{C-Rank} dar.
\\
\\
Auf den ersten Blick scheinen die Werte für \textit{C-Rank} gut zu sein: mehrere Cluster haben eine hohe \textit{Purity} und niedrige \textit{Entropy}.
...
Das semantische Ähnlichkeitsmaß betrachtet viel mehr Relationen einer Publikationen und kommt deshalb auch bei teilweise dünn besetzten Datensätzen zu Ergebnissen.
\\
\\
Bei den drei Metadatengewichtungen des semantischen Ähnlichkeitsmaßes sind die Clustergrößen viel homogener als bei \textit{C-Rank}.
Deshalb würden im Allgemeinen mehrere Cluster mit guten Werten für \textit{Entropy} und \textit{Purity} auch mehr zu den Gesamtwerten beitragen.
