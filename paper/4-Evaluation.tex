...
\textbf{Output:} & \text{Der Ähnlichkeitswert } s(a,b), \forall a,b \in G \text{; \textit{a}, \textit{b} Knoten der selben Art}
\end{array}
\]
...
\textbf{while} \begin{math}(k>0)\end{math} \textbf{do} \hspace{6.2cm}\textbf{/* Iteration */}\\
    ...
    \textbf{foreach} \begin{math}a{\in}G\end{math} \textbf{do}\\
        \textbf{foreach} \begin{math}b{\in}G\end{math} \textbf{do}\\
            \begin{math}in{\longleftarrow}0 \end{math}\\
            \textbf{foreach} \begin{math}i_a{\in}I(a)\end{math} \textbf{do}\\
                \textbf{foreach} \begin{math}i_b{\in}I(b)\end{math} \textbf{do}\\
                    \begin{math}in{\longleftarrow}in{+}R(i_a,i_b)\end{math}\\
            \begin{math}R_{new}(a,b){\longleftarrow}\lambda*\cfrac{c*in}{|I(a)||I(b)|} \end{math}\\
            \begin{math}out{\longleftarrow}0 \end{math}\\
            \textbf{foreach} \begin{math}o_a{\in}O(a)\end{math} \textbf{do}\\
                \textbf{foreach} \begin{math}o_b{\in}O(b)\end{math} \textbf{do}\\
                    \begin{math}out{\longleftarrow}out{+}R(o_a,o_b)\end{math}\\
            \begin{math}R_{new}(a,b){+=}(1-\lambda)*\cfrac{c*out}{|O(a)||O(b)|} \end{math}\\
