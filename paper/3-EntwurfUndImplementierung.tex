...
Zum Vergleich werden hier zwei Codeausschnitte aufgeführt (siehe Listing \ref{pro:graphml} und \ref{pro:owl}), die die \textit{OWL}- bzw. \textit{GraphML}-Repräsentation der Beispielspublikation \textit{Publication\_123456} mit ihren Autor \textit{John Smith} und 2 Keywords \textit{graph theory} und \textit{shortest path} darstellen.
\\
\\
Wie bereits am Code zu erkennen ist, werden bei der \textit{GraphML}-Repräsentation jede Publikation und jedes dazugehörige Metadatum als Knoten eines Graphen modelliert.
Die Klasse, zu der jedes Metadatum gehört, wird dann als Knotenattribut gespeichert.
Die Beziehungen zwischen einer Publikation und einem Metadatum stellen die Kanten des Graphen dar.
Die Art der Beziehung wird wieder als Attribut modelliert, jedoch der Kante.
Auf eine explizite Modellierung beider Relationen, die zwischen einer Publikation und einem Metadatum halten (z.B. \textit{hasAuthor} und \textit{isAuthorOf}), wird um Speicherplatz zu sparen verzichtet.
