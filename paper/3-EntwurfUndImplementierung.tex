...
An der Stelle wird kurz erwähnt, dass die \textit{OWL}-Modellierung nicht die Wissensrepräsentation ist, die auch tatsächlich in der aktuellen Arbeit für die weiteren Ähnlichkeitsanalysen verwendet wird.
...
Die eigentliche Analyse wird im Anschluss genau auf die Datensätze ausgewertet und zwar aus zweierlei Gründen.
\\
\\
Zu einem ist das Ziel dieser Arbeit, festzustellen inwiefern eine semantische Ähnlichkeitsanaylse von wissenschaftlichen Publikationen sinnvolle Ergebnisse liefert.
Hierfür ist das Vorhandensein von einem möglichst vollständigen Satz von Metadaten von besonderer Bedeutung.
Es ist logisch, dass wenn wir eine Ähnlichkeitsanalyse aufgrund von einer gewichteten Kombination von 5 verschiedenen Arten von Metadaten durchführen, werden Papers, für die nicht alle 5 Arten von Metadaten vorliegen, in dem Sinne benachteiligt, dass sie als weniger ähnlich zu anderen Publikationen gewertet werden im Vergleich zu Publikationspaaren, für die vollständige Metadaten da sind.
\\
\\
Zum anderen wird dieses Trimming/Pruning vom Datensatz wieder aus Effizienz- und Resourcenspargründen vorgenommen: mit $4.8$ Gb ist die \textit{GraphML}-Repräsentation des gesamten \textit{zmath}-Datensatzes immer noch zu umfangreich, um mit unseren Methoden effizient ausgewertet zu werden.
Selbst wenn wir die Eingangsdaten in einer Datenbank speichern und nicht in den Speicher laden müssen, haben wir immer noch die Probleme mit $O(n^4)$ Laufzeit auf 3 Mio Knoten und der Erstellung einer Ähnlichkeitsmatrix, die im schlimmsten Fall $O(n^2)$ Platz verbraucht.
